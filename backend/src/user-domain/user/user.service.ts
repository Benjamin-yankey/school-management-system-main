...
import { NotificationService } from "../../notification-domain/notification/notification.service";
...
export class UserService {
  ...
  // ── Direct in-process calls (replace the former HTTP/Kafka service hops) ────

  /** Confirm a school exists. */
  ...
  /** Verify a password against the auth module. */
  private async authVerifyPassword(
    userId: string,
    plainPassword: string,
  ): Promise<{ valid: boolean }> {
    return this.authService.verifyPassword(userId, plainPassword);
  }

  /** Create login credentials in the auth module. */
  ...
  /** Update login credentials in the auth module. */
  ...
  /**
   * Send an email via the notification module.
   * Fire-and-forget: email delivery must never block or fail user provisioning.
   */
  private notifyEmail(data: {
    type: "welcome" | "password-reset";
    to: string;
    tempPassword?: string;
  }): void {
    this.notificationService
      .handleEmailEvent(data as any)
      .catch((err) =>
        console.error("[UserService] notification.email failed:", err),
      );
  }
