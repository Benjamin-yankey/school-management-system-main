...
export class NotificationService {
  ...
  constructor(
    ...
    // Monolith: role/section broadcasts call this app's own public endpoints
    // (carrying the sender's token). All of these resolve to the same base URL.
    const self =
      this.config.get<string>('SELF_URL') ||
      `http://localhost:${process.env.PORT || 3000}`;
    this.apiGatewayUrl = this.config.get<string>('API_GATEWAY_URL') || self;
    this.userServiceUrl = this.config.get<string>('USER_SERVICE_URL') || self;
    this.schoolServiceUrl = this.config.get<string>('SCHOOL_SERVICE_URL') || self;
    this.useGateway = this.config.get<string>('USE_GATEWAY') !== 'false';
    console.log(`[NotificationService] Config: USE_GATEWAY=${this.useGateway}, BASE=${self}`);
  }
